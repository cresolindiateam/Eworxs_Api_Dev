...
$app->post("/TestPdf", function ($request, $response, $args) {
    sendEmail1('cresoluser@gmail.com','demo_invoice_20230825101220.pdf','dd');
});
          ...
          else
          {
             if ($data->getError() === UPLOAD_ERR_OK) 
            {
                ...
                if ($filename) 
                {
                    ...
                    if ($exeInsert) 
                    {
                      $invoiceSql1 ="SELECT client_visites.id as visit_id,client_visites.client_name,client_visites.distance,client_visites.visit_date,company_clients.company_name,company_clients.work_rate,company_clients.mileage_rate,company_clients.postal_code AS ca_postal, workers.postal_code AS ic_postal,company_clients.due_date_range,company_clients.email,companies.send_invoice_status_client,companies.email as company_email,workers.company_id,client_visites.pdf_file FROM `client_visites`  JOIN workers ON(client_visites.employee_id=workers.id) JOIN company_clients ON(client_visites.company_id=company_clients.id) 
...
function add_visit_images($data, $data1)
{
    ...
    if (userauthorization())
     ...
     {
         $invoiceSql1 =
                        "SELECT client_visites.id as visit_id,client_visites.client_name,client_visites.distance,client_visites.visit_date,company_clients.company_name,company_clients.work_rate,company_clients.mileage_rate,company_clients.postal_code AS ca_postal, workers.postal_code AS ic_postal,company_clients.due_date_range,company_clients.email,companies.send_invoice_status_client,companies.email as company_email,workers.company_id FROM `client_visites`  JOIN workers ON(client_visites.employee_id=workers.id) JOIN companies ON(companies.id=workers.company_id)
        ...
        else 
        {
           ...
           $terms='Due on receipt';
            ...
            $pdfHtml =
        		...
        		<tr>
        			<td style="padding-left: 20px; padding: 10px;"><b>Travel time</b> <div style="color:#4c4b49;">' .$dataResult[0]["ic_postal"] ." TO " .$dataResult[0]["ca_postal"] .'</div></td> 
...
function sendInvoiceAddVisit($visit_id)
{
    ...
    $invoiceSql =
        "SELECT client_visites.duration,client_visites.id as visit_id,client_visites.client_name,client_visites.distance,client_visites.visit_date,company_clients.company_name,company_clients.work_rate,company_clients.mileage_rate,company_clients.postal_code AS ca_postal, workers.postal_code AS ic_postal,company_clients.due_date_range,company_clients.email,companies.send_invoice_status_client,companies.email as company_email,company_clients.company_id,company_clients.office_address as client_address,company_clients.postal_code as client_postal_code,companies.company_name as admin_company_name,companies.postal_code as admin_postal_code,companies.address as admin_company_address,client_visites.IMAGE  FROM `client_visites` JOIN workers ON(client_visites.employee_id=workers.id) JOIN company_clients ON(client_visites.company_id=company_clients.id) 
...
function sendEmail1($email, $fileName = "", $fileName1 = "")
{
    ...
    if ($fileName != "" && $fileName1 != "" && $loop1 == true) 
    {
        ...
        $file = "pdfs/" . $fileName;
$file2 = 'https://storage.cloud.google.com/eworxsmobileattachment/banner9.png';
$htmlContent = "<h1>Invoice has been sent.</h1>";
$headers = "From: $fromName" . " <" . $from . ">";
$semi_rand = md5(time());
$mime_boundary = "==Multipart_Boundary_x{$semi_rand}x";
...
if (!empty($file2)) 
{
    ...
    // Check if $file2 is a URL or a local file
    if (filter_var($file2, FILTER_VALIDATE_URL)) {
        // Handle remote URL attachment
    ...
    if ($data2 === false) {
        ...
        // For example:
         echo "Error fetching image: " . error_get_last()['message'];
    }
    ...
    if (isset($data2)) {
        ...
        $message .= "Content-Description: " . basename($file2) . "\n";
        $message .= "Content-Disposition: attachment;\n";
        $message .= " filename=\"" . basename($file2) . "\"; size=" . strlen($data2) . ";\n";
