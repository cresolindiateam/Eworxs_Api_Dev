...
function pending_visits($data)
{
 ...
 if($data["workerId"] == "") 
    {
        ...
        $input_array = [];
        $input_array["employeeId"] = $data["workerId"];
 ...
 {
        $employee_id = $data["workerId"];
        $PendingVisitStatus = 0;
  
            $sqlInsert1= "select id,arrival_time,visit_date,client_name,visit_status from client_visites where employee_id=".$employee_id." and visit_status=".$PendingVisitStatus." order by id desc limit 1";
 ...
 else
 {
	    ...
	    $departure_lang = $data["departureLang"];
          ...
          $client_name = $data["clientName"];
			$sqlInsert1= "select arrival_time, visit_address,employee_id from client_visites where id=".$visit_id."";
				    $exeInsert1 = $db->query($sqlInsert1);
			  $dataResult11 = [];
			                        if ($exeInsert1->num_rows > 0)
			                        {
		...
		if($dataResult11[0]['visit_address']!=$data["departureLocation"])
		{
			 $data = [
		            "status" => false,
		            "message" => "sorry ! can not add this visit"
            ...
            if($departure_time > $arrival_time) 
            {
             ...
             $data = [
				...
				$distance = distance_calculator(
				                $dataResult11[0]['employee_id'],
				                $data["departureLat"],
				                $data["departureLang"]
				            );
				//echo $distance;die;

					    $sqlInsert= "update client_visites set duration=".$duree.", departure_time='".$data["departureTime"]."' where id=".$visit_id."";
                      
					    $exeInsert = $db->query($sqlInsert);



					    $sqlInsert1= "update client_visites set visit_status='1' where id=".$visit_id."";
					    $exeInsert1 = $db->query($sqlInsert1);
					    

				         $sqlInsert= "update client_visites set distance='".$distance."' where id=".$visit_id."";
					    $exeInsert = $db->query($sqlInsert);
...
elseif($data["clientName"] == "") 
 ...
 else
 {
	    ...
	    $sqlInsert =
	        "insert into client_visites(arrival_time,employee_id,company_id,client_name,visit_date,visit_address,visit_lat,visit_lang,distance)" .
	        " VALUES('" .
	        $arrival_time .
	        "','" .
	        $worker_id .
                ...
                if ($data->getError() === UPLOAD_ERR_OK)
                 {
                    ...
                    if ($filename!='') 
                    {
                        ...
                        {
                            $invoiceSql1 =
                                "SELECT client_visites.pdf_file,client_visites.id as visit_id,client_visites.client_name,client_visites.distance,client_visites.visit_date,company_clients.company_name,company_clients.work_rate,company_clients.mileage_rate,client_visites.visit_address AS ca_postal, workers.postal_code AS ic_postal,company_clients.due_date_range,company_clients.email,companies.send_invoice_status_client,companies.email as company_email,workers.company_id FROM `client_visites`  JOIN workers ON(client_visites.employee_id=workers.id) JOIN companies ON(companies.id=workers.company_id)
                                  JOIN company_clients ON(client_visites.company_id=company_clients.id) WHERE client_visites.id=" .
                                $data1;
...
$subject = "Eworxs Invoice";
$message = "<h1>Please find your attachment here .<a href=".$fileName1.">click here </a></h1>";

// Boundary string for separating email parts
...
$imageFile = $fileName1; // Replace with the actual image file path
$imageExtention = pathinfo($fileName1, PATHINFO_EXTENSION);
$imageContent = file_get_contents($imageFile);
$imageBase64 = base64_encode($imageContent);
 $body .= "--$boundary\r\n";
    $body .= "Content-Type: image/$imageExtention; name=\"external-image\"\r\n";
    $body .= "Content-Transfer-Encoding: base64\r\n";
    $body .= "Content-Disposition: attachment; filename=\"external-image\"\r\n\r\n";
    $body .= chunk_split($imageBase64) . "\r\n";

// Attach the PDF file
$pdfFile = "pdfs/" . $fileName;; // Replace with the actual PDF file path
  ...
  if($worker_id == "") 
   ...
   else
   {
        if (userauthorization()) 
        {
            ...
            $sql =
                "SELECT client_visites.id as id,client_visites.company_id as companyClientId,companies.company_name as clientCompanyName,client_visites.client_name as clientName,client_visites.visit_date as visitDate,CONCAT(client_visites.duration, ' h')  AS duration,client_visites.distance,client_visites.pdf_file as pdfFile FROM client_visites left join company_clients on company_clients.id=client_visites.company_id left join companies on companies.id = company_clients.company_id where client_visites.departure_time NOT LIKE '00:00:00' and client_visites.visit_status = 1 and client_visites.employee_id='".$worker_id."' ORDER BY client_visites.id desc LIMIT ".$offset.", ".$limit."";
